...
public class GameSparksSettingsEditor : Editor
{
	...
	GUIContent previewLabel = new GUIContent("Development Build [?]:", "Run app against the preview service");
	...
	public static GameSparksSettings Instance
	{
		get
		{
			...
			{
				instance = Resources.Load(GameSparksSettings.gamesparksSettingsAssetName) as GameSparksSettings;
				if (ReferenceEquals(instance, null))
				{
					// If not found, autocreate the asset object.
					instance = CreateInstance<GameSparksSettings>();
					string properPath = Path.Combine(Application.dataPath, GameSparksSettings.gamesparksSettingsPath);
					
					if (!Directory.Exists(properPath))
					{
						AssetDatabase.CreateFolder("Assets/GameSparks", "Resources");
					}
					
					string fullPath = Path.Combine(Path.Combine("Assets", GameSparksSettings.gamesparksSettingsPath),
