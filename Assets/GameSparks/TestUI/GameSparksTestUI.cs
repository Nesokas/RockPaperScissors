...
public class GameSparksTestUI : MonoBehaviour
{
	...
	void OnGUI ()
	{
		...
		if (GUILayout.Button ("oldAccountDetailsRequest", GUILayout.Width (itemWidth), GUILayout.Height (itemHeight))) {
			GameSparks.GameSparksApi.accountDetailsRequest().send();
		}
		...
		if (GUILayout.Button ("authenticationRequest", GUILayout.Width (itemWidth), GUILayout.Height (itemHeight))) {
			AuthenticationResponse AR = new AuthenticationRequest ().SetUserName ("gabs").SetPassword ("gabs").Send ();
			if (AR.HasErrors) {
		...
		if (GUILayout.Button ("leaderboardData", GUILayout.Width (itemWidth), GUILayout.Height (itemHeight))) {
			LeaderboardDataResponse leadResponse = new LeaderboardDataRequest ().SetLeaderboardShortCode ("HSCORE").SetEntryCount (10) .Send ();
